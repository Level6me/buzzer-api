...
支持常见的 **HS-F02P 无源蜂鸣器模块**（PH2.0 3P 接口，工作电压 3.3V ~ 5.5V）。

### 实物接线与简易原理图

![HS-F02P 无源蜂鸣器实物接线图 & 简易原理图](docs/images/buzzer_simple_schematic.png)

### 树莓派 40-Pin 引脚对插定义

模块正面丝印线序为从左到右：**G | V | S**，使用随附的 PH2.0 转杜邦线直插树莓派物理引脚即可：

| 导线颜色 | 模块端子丝印 | 树莓派物理引脚 | 说明 |
|:---:|:---:|:---:|:---|
| **🟢 绿线** | **G** | **Pin 6 (GND)** | 信号与电源公共地线（亦可接 Pin 14） |
| **🔴 红线** | **V** | **Pin 2 (5V)** | 推荐接 5V 核心供电，音量饱满充沛（接 Pin 1 3.3V 为弱音） |
| **🔵 蓝线** | **S** | **Pin 12 (GPIO18)** | 硬件 PWM0 脉冲信号线 |

> 提示：模块自带板载 `Power` 红色电源指示灯，正确接好 VCC 与 GND 后即会常亮，方便确认通电状态。

<details>
<summary><b>🔍 点击展开：完整驱动电路与保护机制原理图（含官方原厂原理图）</b></summary>

![HS-F02P 模块驱动与保护电路原理图](docs/images/buzzer_hardware_schematic.png)

#### 官方原厂原理图 (Hello STEM HS-F02P REV 1.0)

![HS-F02P 官方原厂原理图](docs/images/official_schematic.png)

* **驱动核心 (Q1)**：SS8050 NPN 开关三极管，瞬态电流 500mA，保护树莓派 GPIO 免受线圈大电流冲击。
* **反峰吸收 (D1)**：BAT60B 超高速低压降肖特基二极管，并在蜂鸣器线圈两端吸收关断反电动势尖峰。
* **基极限流 (R2)**：47Ω 贴片限流电阻，降低驱动内阻，提升高频响应。
* **电源指示 (LED1)**：0603 红色高亮 LED + 1kΩ 限流电阻，直观显示 VCC 供电状态。
</details>

> 本服务将 GPIO18 设为 **ALT5（PWM0）**，信号由树莓派底层硬件时钟发生器驱动，CPU 占用为 0，音调纯净无抖动。
